...
class path_planningNode:
    def __init__(self):
        ...
        self.twist_cur_.twist.linear.x = 0.5
        self.twist_cur_.twist.linear.y = 0.5
        self.twist_cur_.twist.linear.z = 0.5
    ...
    def comparePosition(self, pose_tar, pose_cur):
        if pose_tar.pose.position.x != pose_cur.pose.position.x:
            return False
        if pose_tar.pose.position.y != pose_cur.pose.position.y:
            return False
        if pose_tar.pose.position.z != pose_cur.pose.position.z:
            return False
        return True
    ...
    def executeCB(self):
        ...
        execute_duration = 1 * (dist / 1)  # Execute interpolation for 5 seconds
        ...
        while not self.comparePosition(pose_tar, self.pose_cur_):
            ...
            if (time_cur.to_sec() - time_start.to_sec()) <= 0:  # If current time is earlier than start time
                ...
                print(j, "[calculated pose_goal] x =", pose_goal.pose.position.x)
                j+=1
        ...
        print(len(self.trajectory_cur_.poses))
        
        for p in range(len(self.trajectory_cur_.poses)):
            print(p, "[trajectory_cur_]", self.trajectory_cur_.poses[p].pose.position.x, self.trajectory_cur_.poses[p].pose.position.y, self.trajectory_cur_.poses[p].pose.position.z)
